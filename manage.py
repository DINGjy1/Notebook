...
def login():
    ...
    return render_template('login.html')   #返回渲染模板
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        password_candidate1 = request.form['password']
        sql = "SELECT * FROM admin  WHERE adminname = '%s' "%(adminname) # 根据用户名查找admin表中记录
        db = MysqlUtil()  # 实例化数据库操作类
        result = db.fetchone(sql)  # 获取一条记录
        if result:  # 如果查到记录
            password = result['password']  # 用户填写的密码
            if password_candidate1 == password:
                session['logged_in'] = True
                ...
                error = '用户名和密码不匹配'
                return render_template('admin_login.html', error=error) # 跳转到登录页，并提示错误信息
        else:
            error = '用户名不存在'
            return render_template('admin_login.html', error=error)
    return render_template('admin_login.html')   #返回渲染模板
...
def admin_dashboard():
    ...
    result = db.fetchall(sql) # 查找所有用户
    if result: # 如果笔记存在，赋值给articles变量
        return render_template('admin_dashboard.html', users=result)
    else:      # 如果笔记不存在，提示暂无笔记
        msg = '暂无用户信息'
...
@is_logged_in
def add_user():
    form = RegisterForm(request.form) # 实例化ArticleForm表单类
    if request.method == 'POST' and form.validate(): # 如果用户提交表单，并且表单验证通过
        # 获取表单字段内容
        username = form.username.data
        email = form.email.data
        ...
        sql = "INSERT INTO user(username,email,password) \
               VALUES ('%s', '%s', '%s')" % (username,email,password) # 插入数据的SQL语句
        db.insert(sql)
        flash('创建成功', 'success') # 闪存信息
        return render_template('admin_dashboard.html')             # 跳转到控制台
...
def edit_article(id):
    ...
    if request.method == 'POST' and form.validate(): # 如果用户提交表单，并且表单验证通过
        ...
        db = MysqlUtil() # 实例化数据库操作类
        db.update(update_sql) # 调用类中upadte()方法，更新数据的SQL语句
...
def edit_user(id):
    db = MysqlUtil()  # 实例化数据库操作类
    fetch_sql = "SELECT * FROM user WHERE id = '%s'" % (id) # 根据笔记ID查找笔记信息
    user = db.fetchone(fetch_sql) # 查找一条记录
    # 检测笔记不存在的情况
    if not user:
        flash('ID错误', 'danger') # 闪存信息
        return render_template('admin_dashboard.html')
    ...
    if request.method == 'POST' and form.validate(): # 如果用户提交表单，并且表单验证通过
        ...
        update_sql = "UPDATE articles SET username='%s', email='%s',password='%s' WHERE id='%s'" % (username, email, password,id)
        db = MysqlUtil() # 实例化数据库操作类
        db.update(update_sql) # 调用类中upadte()方法，更新数据的SQL语句
        flash('更改成功', 'success') # 闪存信息
        return render_template('admin_dashboard.html')  # 跳转到控制台
...
def delete_user(id):
    db = MysqlUtil() # 实例化数据库操作类
    sql = "DELETE FROM user WHERE id = '%s'" % (id,session['username']) # 执行删除笔记的SQL语句
    db.delete(sql) # 删除数据库
    flash('删除成功', 'success') # 闪存信息
    return render_template('admin_dashboard.html')  # 跳转到控制台
